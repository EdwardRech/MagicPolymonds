Program Perfect_Magic_Polymonds;
Uses Polymonds, Graph, WinCRT;
...
Procedure Find_Polymonds(var j:Smallint; var LastMove,MoveList:A; var CurX,CurY:LongInt);
var i, MoveNum: Smallint;
Begin
	inc(j);
	MoveNum:=0;
	For i:=1 to 4 do Begin
		inc(MoveNum);
		Move(LastMove[j-1],CurX,CurY,MoveNum,j,MoveList,LastMove[j]);
		If j<>n then Find_Polymonds(j,LastMove,MoveList,CurX,CurY)
		Else Begin
			If ((CurX=0) and (CurY=0)) and ((LastMove[j]<>LastMove[1]) and (LastMove[j]<>LastMove[1]-3)) then Write(f, MoveList);
		End;
		ReVmove(LastMove[j],CurX,CurY,j);
	End;
	dec(j);
End;
...
Begin
	...
	End;
	CurX:=1; CurY:=-1; 
	MoveList[1]:=1; LastMove[1]:=4;
	MoveNum:=1; j:=2;
	Assign(f,'All_Polymonds_Moves.dat'); Rewrite(f);
	For i:=2 to 3 do Begin
		inc(MoveNum);
		Move(LastMove[1],CurX,CurY,MoveNum,2,MoveList,LastMove[2]);
		Find_Polymonds(j,LastMove,MoveList,CurX,CurY);
		RevMove(LastMove[2],CurX,CurY,2);
	End;
	Close(f);
	...
	Reset(f2); Rewrite(f3);
	Writeln('We found ',CrossChecking,' perfect magic polymonds');
	Close(f2); Close(f3);
	...
	Else Begin
		...
		While not EoF(f3) do Begin
			...
			For j:=1 to i do Begin 
				MoveTo(LastMove[j-1]*Koef+Koef2x,Movelist[j-1]*Koef+Koef2y);
				LineTo(LastMove[j]*Koef+Koef2x,Movelist[j]*Koef+Koef2y);
			End;
			Readkey;
			CloseGraph; 
		End;
